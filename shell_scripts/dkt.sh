...
# Experiment mode (passed as argument: "ablation" or "sweep" or "both")
EXP_MODE=${1:-"both"}

# Ablation configurations
# 0: Baseline (no MP)
# 8: +MP (MP 0-3 + Ratio 0-3)
...
# Best hyperparameters for ablation (use after sweep)
BEST_LR=${2:-"1e-3"}
BEST_DROPOUT=${3:-"0.05"}
...
run_single_experiment() {
    ...
    echo "  [DKT] fold=$fold, lr=$lr, dropout=$dropout, use_mp=$use_mp"
...
run_ablation_study() {
    ...
    for use_mp in "${MP_CONFIGS_ABLATION[@]}"; do
        ...
        # Clean up saved models to save disk space
...
run_hyperparameter_sweep() {
    echo "=========================================="
    echo "DKT Hyperparameter Sweep"
    echo "=========================================="

    local use_mp=0  # Baseline for hyperparameter search
    ...
    for dropout in "${DROPOUTS[@]}"; do
        ...
        # Clean up after each dropout setting
        rm -rf ${SAVE_DIR}/${DATASET_NAME}_${MODEL_NAME}_qid_saved_model_${SEED}_*
    done
}
...
cd "$(dirname "$0")/../pykt-toolkit/examples"  # Navigate to pykt-toolkit/examples

echo "Starting DKT experiments..."
echo "Mode: $EXP_MODE"
...
echo "DKT experiments completed!"
