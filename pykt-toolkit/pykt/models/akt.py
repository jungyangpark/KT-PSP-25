...
class AKT(nn.Module):
    ...
    def forward(self, q_data, target, pid_data=None, qtest=False, dcur=None):
        ...
        # Ratio predictions (use_mp=8일 때 multi-task, training only)
        if self.use_mp == 8 and not qtest:
            # Multi-task: Ratio 0-3 predictions (4개) - only during training
            ratio_predictions = []
            for i in range(4):  # Ratio 0-3 예측
                ratio_pred = self.ratio_out_layers[i](concat_q)  # Shape: (batch, seq, num_c)
                ratio_pred = m(ratio_pred)  # [0, 1] range for ratio
                ratio_predictions.append(ratio_pred)
            return preds, c_reg_loss, ratio_predictions
        else:
            # Evaluation or single task: 정오답 예측만
            if not qtest:
