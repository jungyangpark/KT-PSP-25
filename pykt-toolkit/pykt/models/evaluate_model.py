...
def evaluate(model, test_loader, model_name, rel=None, save_path="", log_dir=None):
    if save_path != "":
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if model_name in ["atdkt"]:
                ...
                outputs = model(dcur)
                # Handle tuple output (knowledge pred, ratio preds) - use only knowledge prediction
                if isinstance(outputs, tuple):
                    y = outputs[0]
                else:
                    y = outputs
                y = y[:,1:]
            elif model_name in ["stablekt", "sparsekt", "cskt", "ukt", "hcgkt"]:
                outputs = model(dcur)
                # Handle tuple output - use only knowledge prediction
                if isinstance(outputs, tuple):
                    y = outputs[0]
                else:
                    y = outputs
                ...
                # Handle tuple output (knowledge pred, MP preds) - use only knowledge prediction
                if isinstance(outputs, tuple):
                    y = outputs[0]
                else:
                    y = outputs
                ...
                # Handle tuple output (y, ratio_preds) - use only y
                ...
                if model_name in ["akt", "robustkt"]:
                    outputs = model(cc.long(), cr.long(), cq.long(), dcur=dcur)
                else:
                    outputs = model(cc.long(), cr.long(), cq.long())
                # Handle tuple output (y, reg_loss) or (y, reg_loss, ratio_preds) - use only y and reg_loss
                ratio_preds = None
                if isinstance(outputs, tuple):
                    y = outputs[0]  # Always use first element (knowledge prediction)
                    # Check if ratio predictions are available (3rd element)
                    if len(outputs) >= 3 and isinstance(outputs[2], list):
                        ratio_preds = outputs[2]  # Ratio predictions (4개)
                else:
                    y = outputs
                y = y[:,1:]

                # Store ratio predictions for first batch (for logging)
                if test_mini_index == 0 and ratio_preds is not None:
                    ratio_preds_shifted = [rp[:,1:] for rp in ratio_preds]
                ...
                outputs = model(c.long(), r.long(), dcur=dcur)
                # Handle tuple output (y, features) or (y, features, ratio_preds) - use only y
                if isinstance(outputs, tuple):
                    y = outputs[0]  # Always use first element (knowledge prediction)
                else:
                    y = outputs
            ...
            # Store detailed information for qualitative analysis (first batch only)
            if test_mini_index == 0:  # First batch for detailed logging
                detailed_batch = {
                    'q': q.detach().cpu(),
                    'c': c.detach().cpu(),
                    'r': r.detach().cpu(),
                    'qshft': qshft.detach().cpu(),
                    'cshft': cshft.detach().cpu(),
                    'rshft': rshft.detach().cpu(),
                    'y_pred': y.clone().detach().cpu(),
                    'sm': sm.detach().cpu()
                }
                # Add student ID (uid) if available
                if 'uids' in dcur and dcur['uids'] is not None:
                    detailed_batch['uids'] = dcur['uids'].detach().cpu()

                # Add MP features if available
                if hasattr(model, 'use_mp') and model.use_mp == 8:
                    detailed_batch['mp_features'] = {}
                    for i in range(4):
                        mp_key = f"math_prof_{i}"
                        if mp_key in dcur and dcur[mp_key] is not None:
                            detailed_batch['mp_features'][mp_key] = dcur[mp_key].detach().cpu()
                    for i in range(4):
                        ratio_key = f"mp_ratio_{i}"
                        if ratio_key in dcur and dcur[ratio_key] is not None:
                            detailed_batch['mp_features'][ratio_key] = dcur[ratio_key].detach().cpu()

                    # Add ratio predictions from model output
                    if model_name in ["akt", "robustkt"] and 'ratio_preds_shifted' in locals():
                        detailed_batch['ratio_predictions'] = [rp.detach().cpu() for rp in ratio_preds_shifted]
        ...
        # Prepare qualitative evaluation log
        log_lines = []
        log_lines.append("=" * 80)
        log_lines.append(f"QUALITATIVE EVALUATION - {model_name.upper()}")
        log_lines.append("=" * 80)
        log_lines.append("")

        # Display all sequences from the first batch
        num_sequences = detailed_batch['q'].shape[0]
        for seq_idx in range(num_sequences):
            log_lines.append(f"[Sequence {seq_idx + 1}]")

            # Add student ID if available
            if 'uids' in detailed_batch:
                student_id = detailed_batch['uids'][seq_idx].item()
                log_lines.append(f"Student ID: {student_id}")

            log_lines.append("-" * 80)

            # Get valid timesteps (where mask is True)
            mask = detailed_batch['sm'][seq_idx].numpy()
            seq_len = int(mask.sum())

            log_lines.append(f"Sequence length: {seq_len}")
            log_lines.append("")
            log_lines.append("Timestep-by-timestep breakdown:")

            # Build header
            header = f"{'T':>3} | {'Question':>8} | {'Concept':>7} | {'Response':>8} | {'Next Q':>8} | {'Next C':>7} | {'True':>4} | {'Pred':>6} | {'Label':>5}"
            if 'mp_features' in detailed_batch:
                header += " | "
                for i in range(4):
                    header += f"MP{i:1d}:{'':>5} | "
                for i in range(4):
                    header += f"R{i:1d}:{'':>5} | "
                # Add ratio prediction headers
                if 'ratio_predictions' in detailed_batch:
                    for i in range(4):
                        header += f"RP{i:1d}:{'':>4}" + (" | " if i < 3 else "")
                else:
                    # Remove last separator if no ratio predictions
                    header = header.rstrip(" | ")
            log_lines.append(header)
            log_lines.append("-" * 200)  # Longer separator for wider table

            # Print each timestep
            for t in range(seq_len):
                q_val = detailed_batch['q'][seq_idx, t].item()
                c_val = detailed_batch['c'][seq_idx, t].item()
                r_val = detailed_batch['r'][seq_idx, t].item()
                qshft_val = detailed_batch['qshft'][seq_idx, t].item()
                cshft_val = detailed_batch['cshft'][seq_idx, t].item()
                rshft_val = detailed_batch['rshft'][seq_idx, t].item()
                y_val = detailed_batch['y_pred'][seq_idx, t].item()
                pred_label = 1 if y_val >= 0.5 else 0

                line = f"{t:3d} | {int(q_val):8d} | {int(c_val):7d} | {int(r_val):8d} | {int(qshft_val):8d} | {int(cshft_val):7d} | {int(rshft_val):4d} | {y_val:6.4f} | {pred_label:5d}"

                # Add MP feature values if available
                if 'mp_features' in detailed_batch:
                    line += " | "
                    for i in range(4):
                        mp_key = f"math_prof_{i}"
                        if mp_key in detailed_batch['mp_features']:
                            mp_val = detailed_batch['mp_features'][mp_key][seq_idx, t].item()
                            line += f"{mp_val:6.3f} | "
                        else:
                            line += f"{'N/A':>6} | "
                    for i in range(4):
                        ratio_key = f"mp_ratio_{i}"
                        if ratio_key in detailed_batch['mp_features']:
                            ratio_val = detailed_batch['mp_features'][ratio_key][seq_idx, t].item()
                            line += f"{ratio_val:6.3f} | "
                        else:
                            line += f"{'N/A':>6} | "

                    # Add ratio prediction values if available
                    if 'ratio_predictions' in detailed_batch:
                        for i in range(4):
                            # ratio_predictions format depends on model:
                            # - RobustKT: list of 4 tensors, each (batch, seq) - already squeezed
                            # - DKT/AKT: list of 4 tensors, each (batch, seq, num_c) - needs concept selection
                            ratio_pred_tensor = detailed_batch['ratio_predictions'][i][seq_idx, t]

                            # Check if tensor is scalar (RobustKT) or has shape (DKT/AKT)
                            if ratio_pred_tensor.dim() == 0:  # RobustKT: scalar tensor
                                ratio_pred_val = ratio_pred_tensor.item()
                                line += f"{ratio_pred_val:6.3f}" + (" | " if i < 3 else "")
                            else:  # DKT/AKT: tensor with shape (num_c,)
                                cshft_val = int(detailed_batch['cshft'][seq_idx, t].item())
                                if cshft_val < ratio_pred_tensor.shape[0]:
                                    ratio_pred_val = ratio_pred_tensor[cshft_val].item()
                                    line += f"{ratio_pred_val:6.3f}" + (" | " if i < 3 else "")
                                else:
                                    line += f"{'N/A':>6}" + (" | " if i < 3 else "")
                    else:
                        # Remove trailing separator if no ratio predictions
                        line = line.rstrip(" | ")
                log_lines.append(line)
            log_lines.append("")

        log_lines.append("Overall Statistics:")
        log_lines.append(f"  Total predictions: {len(ps)}")
        log_lines.append(f"  Predictions - min={ps.min():.4f}, max={ps.max():.4f}, mean={ps.mean():.4f}, std={ps.std():.4f}")
        log_lines.append(f"  Ground truth - distribution: {np.bincount(ts.astype(int))}")
        log_lines.append("=" * 80)
        log_lines.append("")

        # Save to file if log_dir is provided
        if log_dir is not None:
            log_path = os.path.join(log_dir, "test_qualitative.log")
            with open(log_path, "w", encoding="utf8") as log_file:
                log_file.write("\n".join(log_lines))
            print(f"Qualitative evaluation saved to: {log_path}")
...
def evaluate_question(model, test_loader, model_name, fusion_type=["early_fusion", "late_fusion"], save_path=""):
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            dcur = dict()
            if model_name == "dkvmn":
                # Use dcur parameter for ED/MP features - DKVMN model handles internal feature extraction
                y, h = model(cc.long(), cr.long(), qtest=True, dcur=dcur)
                y = y[:,1:]
            elif model_name in ["skvmn"]:
                # Use dcur parameter for ED features - SKVMN model handles internal ED extraction
                outputs = model(cc.long(), cr.long(), qtest=True, dcur=dcur)
                # Handle tuple output - qtest=True returns (y, h) or just y
                if isinstance(outputs, tuple):
                    y, h = outputs[0], outputs[1]  # (y, h) from qtest
                else:
                    y = outputs
                y = y[:,1:]
            elif model_name in ["deep_irt"]:
                ...
                outputs = model(dcurori, qtest=True, train=False)
                # Handle tuple output - qtest=True returns (y, h) or just y
                if isinstance(outputs, tuple):
                    y, h = outputs[0], outputs[1]  # (y, h) from qtest
                else:
                ...
                outputs = model(dcurori, qtest=True, train=False)
                # Handle tuple output - qtest=True returns (y, h)
                if isinstance(outputs, tuple):
                    y, h = outputs[0], outputs[1]
                else:
                    y = outputs
                y = y[:,1:]
            elif model_name in ["rekt"]:
                ...
                # Use dcur parameter for ED features - AKT/RobustKT models handle internal ED extraction
                if model_name in ["akt", "robustkt"]:
                    y, reg_loss, h = model(cc.long(), cr.long(), cq.long(), qtest=True, dcur=dcur)
                else:
                ...
                outputs = model(cq.long(), cc.long(), r.long(), qtest=True, dcur=dcur)
                # Handle tuple output - qtest=True returns (y, h) or just y
                if isinstance(outputs, tuple):
                    y, h = outputs[0], outputs[1]  # (y, h) from qtest
                else:
                ...
                outputs = model(c.long(), r.long(), dgaps)
                # Handle tuple output (knowledge pred, ratio preds) - use only knowledge prediction
                if isinstance(outputs, tuple):
                    y = outputs[0]
                else:
...
def predict_each_group(dtotal, dcur, dforget, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad=False, maxlen=200):
    ...
    for k in range(t, end):
        ...
        if model_name in ["atdkt"]: ## need change!
            ...
            output = model(cin.long(), rin.long(), cshft.long())
            # Handle tuple output (y, ratio_preds) - use only y
            if isinstance(output, tuple):
                y = output[0]
            else:
            ...
            outputs = model(qin.long(), cin.long(), rin.long())
            # Handle tuple output (knowledge pred, ratio preds) - use only knowledge prediction
            if isinstance(outputs, tuple):
                y = outputs[0]
            else:
            ...
            outputs = model(cin.long(), rin.long())
            # Handle tuple output (y, features) or (y, features, ratio_preds) - use only y
            if isinstance(outputs, tuple):
                y = outputs[0]
            else:
            ...
            # Use dcur parameter for DKVMN/SKVMN models - they handle internal ED/MP extraction
            # For interactive prediction, dcur is empty so models will use -1 padding
            if model_name in ["dkvmn", "skvmn"]:
                outputs = model(cin.long(), rin.long(), dcur={})
                # Handle tuple output (knowledge pred, ratio preds) - use only knowledge prediction
                if isinstance(outputs, tuple):
                    y = outputs[0]
                else:
            ...
            outputs = model(cin.long(), rin.long(), qin.long())
            # Handle tuple output (y, reg_loss) or (y, reg_loss, ratio_preds) - use only y
            if isinstance(outputs, tuple):
                y = outputs[0]
            else:
                y = outputs
...
def predict_each_group2(dtotal, dcur, dforget, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad=False, maxlen=200):
    ...
    while bidx < finalcs.shape[0]:
        ...
        if model_name in ["atdkt"]:
            ...
            outputs = model(curc.long(), curr.long(), dgaps)
            # Handle tuple output (knowledge pred, ratio preds) - use only knowledge prediction
            if isinstance(outputs, tuple):
                y = outputs[0]
            else:
            ...
            # Use dcur parameter for ED/MP features - DKVMN model handles internal feature extraction
            outputs = model(ccc.long(), ccr.long(), dcur=curd)
            # Handle tuple output (knowledge pred, ratio preds) - use only knowledge prediction
            if isinstance(outputs, tuple):
                y = outputs[0]
            else:
            ...
            output = model(curc.long(), curr.long(), curcshft.long())
            # Handle tuple output (y, ratio_preds) - use only y
            if isinstance(output, tuple):
                y = output[0]
            else:
            ...
            outputs = model(ccq.long(), ccc.long(), curr.long())
            # Handle tuple output (knowledge pred, ratio preds) - use only knowledge prediction
            if isinstance(outputs, tuple):
                y = outputs[0]
            else:
            ...
            outputs = model(ccc.long(), ccr.long(), ccq.long())
            # Handle tuple output (y, reg_loss) or (y, reg_loss, ratio_preds) - use only y
            if isinstance(outputs, tuple):
                y = outputs[0]
            else:
            ...
            outputs = model(curc.long(), curr.long())
            # Handle tuple output (y, features) or (y, features, ratio_preds) - use only y
            if isinstance(outputs, tuple):
                y = outputs[0]
            else:
            ...
            outputs = model(ccc.long(), ccr.long())
            # Handle tuple output (knowledge pred, ratio preds) - use only knowledge prediction
            if isinstance(outputs, tuple):
                y = outputs[0]
            else:
