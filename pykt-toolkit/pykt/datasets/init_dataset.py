...
def init_test_datasets(data_config, model_name, batch_size, diff_level=None, args=None, re_mapping=False, cold_start_len=0):
    dataset_name = data_config["dataset_name"]
    print(f"model_name is {model_name}, dataset_name is {dataset_name}")
    if cold_start_len > 0:
        print(f"[Cold-start] Evaluation using only first {cold_start_len} elements")
    ...
    if model_name in ["dkt_forget", "bakt_time"]:
        ...
        test_dataset = KTDataset(os.path.join(data_config["dpath"], data_config["test_file"]), data_config["input_type"], {-1}, cold_start_len=cold_start_len)
        test_window_dataset = KTDataset(os.path.join(data_config["dpath"], data_config["test_window_file"]), data_config["input_type"], {-1}, cold_start_len=cold_start_len)
        if "test_question_file" in data_config:
            test_question_dataset = KTDataset(os.path.join(data_config["dpath"], data_config["test_question_file"]), data_config["input_type"], {-1}, True, cold_start_len=cold_start_len)
            test_question_window_dataset = KTDataset(os.path.join(data_config["dpath"], data_config["test_question_window_file"]), data_config["input_type"], {-1}, True, cold_start_len=cold_start_len)
