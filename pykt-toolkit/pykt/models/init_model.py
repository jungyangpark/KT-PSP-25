...
def init_model(model_name, model_config, data_config, emb_type, use_ed=0, use_mp=0):
    if model_name == "dkt":
        model = DKT(data_config["num_c"], **model_config, emb_type=emb_type, emb_path=data_config["emb_path"], use_ed=use_ed, use_mp=use_mp).to(device)
    elif model_name == "dkt2":
        model = DKT2(data_config["num_c"], **model_config, emb_type=emb_type, emb_path=data_config["emb_path"], use_ed=use_ed, use_mp=use_mp).to(device)
        ...
        model = DisKT(data_config["num_c"], **diskt_config, emb_type=emb_type, emb_path=data_config["emb_path"], use_ed=use_ed, use_mp=use_mp).to(device)
    elif model_name == "dkt+":
        model = DKTPlus(data_config["num_c"], **model_config, emb_type=emb_type, emb_path=data_config["emb_path"], use_ed=use_ed, use_mp=use_mp).to(device)
    elif model_name == "dkvmn":
        model = DKVMN(data_config["num_c"], **model_config, emb_type=emb_type, emb_path=data_config["emb_path"], use_ed=use_ed, use_mp=use_mp).to(device)
        ...
        model = SAKT(data_config["num_c"],  **model_config, emb_type=emb_type, emb_path=data_config["emb_path"], use_ed=use_ed, use_mp=use_mp).to(device)
    elif model_name == "saint":
        model = SAINT(data_config["num_q"], data_config["num_c"], **model_config, emb_type=emb_type, emb_path=data_config["emb_path"], use_ed=use_ed, use_mp=use_mp).to(device)
    elif model_name == "dkt_forget":
        model = DKTForget(data_config["num_c"], data_config["num_rgap"], data_config["num_sgap"], data_config["num_pcount"], **model_config, use_ed=use_ed, use_mp=use_mp).to(device)
    elif model_name == "akt":
        model = AKT(data_config["num_c"], data_config["num_q"], **model_config, emb_type=emb_type, emb_path=data_config["emb_path"], use_ed=use_ed, use_mp=use_mp).to(device)
        ...
        model = ATKT(data_config["num_c"], **model_config, emb_type=emb_type, emb_path=data_config["emb_path"], fix=False, use_ed=use_ed, use_mp=use_mp).to(device)
    elif model_name == "atktfix":
        model = ATKT(data_config["num_c"], **model_config, emb_type=emb_type, emb_path=data_config["emb_path"], fix=True, use_ed=use_ed, use_mp=use_mp).to(device)
    elif model_name == "gkt":
        ...
        model = GKT(data_config["num_c"], **model_config,graph=graph,emb_type=emb_type, emb_path=data_config["emb_path"], use_mp=use_mp).to(device)
        ...
        model = SKVMN(data_config["num_c"], **model_config, emb_type=emb_type, emb_path=data_config["emb_path"], use_ed=use_ed, use_mp=use_mp).to(device)   
        ...
        model = simpleKT(data_config["num_c"], data_config["num_q"], **model_config, emb_type=emb_type, emb_path=data_config["emb_path"], use_ed=use_ed, use_mp=use_mp).to(device)
        ...
        model = stableKT(data_config["num_c"], data_config["num_q"], **model_config, emb_type=emb_type, emb_path=data_config["emb_path"], use_ed=use_ed, use_mp=use_mp).to(device)
        ...
        model = Robustkt(data_config["num_c"], data_config["num_q"], **model_config, emb_type=emb_type, emb_path=data_config["emb_path"], use_ed=use_ed, use_mp=use_mp).to(device)
...
def load_model(model_name, model_config, data_config, emb_type, ckpt_path, use_ed=0):
    model = init_model(model_name, model_config, data_config, emb_type, use_ed=use_ed)
