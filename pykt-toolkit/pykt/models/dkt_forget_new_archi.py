...
class DKTForget(Module):
    def __init__(self, num_c, num_rgap, num_sgap, num_pcount, emb_size, dropout=0.1, emb_type='qid', emb_path="", use_ed=0, use_mp=0):
        ...
        if self.use_mp == 8:
            ...
            # Step 2: Ratio 4*num_c개를 입력으로 받아 prediction 예측하는 FC layer
            # Input: out_input_size + num_c*4 (ratio predictions) -> Output: num_c (prediction)
            self.out_layer = Linear(out_input_size + self.num_c*4, self.num_c)
        else:
    ...
    def forward(self, q, r, dgaps, dcur=None):
        ...
        if self.use_mp == 8:
            ...
            for i in range(4):  # Ratio 0-3 예측
                ratio_pred = self.ratio_out_layers[i](theta_out)  # Shape: (batch, seq, num_c)
                ratio_pred = torch.sigmoid(ratio_pred)  # [0, 1] range for ratio
                ratio_predictions.append(ratio_pred)

            # Step 2: Concatenate ratio predictions to theta_out
            ...
            # Step 3: Predict final output using theta_out + ratio predictions
            theta_out_with_ratio = torch.cat([theta_out, ratio_concat], dim=-1)  # Shape: (batch, seq, out_input_size + num_c*4)
            y = self.out_layer(theta_out_with_ratio)  # Shape: (batch, seq, num_c)
            y = torch.sigmoid(y)
