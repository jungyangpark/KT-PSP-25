    ...
    def __getitem__(self, index):
        ...
        for key in self.dori:
            ...
            if len(self.dori[key]) == 0:
                print(f"[DKT_forget Dataloader] Warning: {key} is empty (len=0), skipping")
            ...
            for j in range(5):
                ...
                if error_col in row and pd.notna(row[error_col]) and row[error_col] != '':
                    ...
                    while len(ed_seq) < len(dori["rseqs"][-1]):
                        ed_seq.append(0.0)
                    dori[error_col].append(ed_seq[:len(dori["rseqs"][-1])])
                else:
                    dori[error_col].append([0.0] * len(dori["rseqs"][-1]))

            # Extract MP features (math_prof_0 to math_prof_3)
            for j in range(4):
                mp_col = f'math_prof_{j}'
                if mp_col in row and pd.notna(row[mp_col]) and row[mp_col] != '':
                    ...
                    while len(mp_seq) < len(dori["rseqs"][-1]):
                        mp_seq.append(0.0)
                    dori[mp_col].append(mp_seq[:len(dori["rseqs"][-1])])
                else:
                    dori[mp_col].append([0.0] * len(dori["rseqs"][-1]))

            # Extract MP Ratio features (mp_ratio_0 to mp_ratio_3)
            for j in range(4):
                ratio_col = f'mp_ratio_{j}'
                if ratio_col in row and pd.notna(row[ratio_col]) and row[ratio_col] != '':
                    ratio_seq = [float(e) for e in str(row[ratio_col]).split(',') if e not in ['-1', '']]
                    while len(ratio_seq) < len(dori["rseqs"][-1]):
                        ratio_seq.append(0.0)
                    dori[ratio_col].append(ratio_seq[:len(dori["rseqs"][-1])])
                else:
                    dori[ratio_col].append([0.0] * len(dori["rseqs"][-1]))
