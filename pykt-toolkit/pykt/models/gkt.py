...
class GKT(nn.Module):
    ...
    def forward(self, q, r, dcur=None):
        ...
        for i in range(seq_len):
            ...
            if self.use_mp == 8 and i < seq_len - 1:
                ...
                for j in range(4):  # Ratio 0-3 예측
                    ratio_pred = self.ratio_out_layers[j](h_next).squeeze(dim=-1)  # [batch_size, num_c, hidden_dim] -> [batch_size, num_c]
                    ratio_pred = torch.sigmoid(ratio_pred)  # [0, 1] range for ratio
                    # dot product between ratio_pred and one_hot_qt
                    ratio_pred_next = (ratio_pred * one_hot_qt).sum(dim=1)  # [batch_size, ]
