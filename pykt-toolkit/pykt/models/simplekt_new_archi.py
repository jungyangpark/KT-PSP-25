    ...
    def __init__(self, n_question, n_pid, 
        ...
        if self.use_mp == 8:
            ...
            self.out = nn.Sequential(
                nn.Linear(final_input_dim + 4, final_fc_dim), nn.ReLU(), nn.Dropout(self.dropout),
                nn.Linear(final_fc_dim, final_fc_dim2), nn.ReLU(), nn.Dropout(self.dropout),
                nn.Linear(final_fc_dim2, 1)
            )
        else:
        ...
        if emb_type in ["qid", "qidaktrasch", "qid_scalar", "qid_norasch"]:
            ...
            if self.use_mp == 8:
                ...
                if not qtest:
                    ...
                    if train:
                        return preds, y2, y3, ratio_predictions_squeezed
                    else:
                        return preds, ratio_predictions_squeezed
                else:
                    # Evaluation with qtest: return prediction + concat_q
