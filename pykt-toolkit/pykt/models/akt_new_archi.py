...
class AKT(nn.Module):
    def __init__(self, n_question, n_pid, d_model, n_blocks, dropout, d_ff=256,
            kq_same=1, final_fc_dim=512, num_attn_heads=8, separate_qa=False, l2=1e-5, emb_type="qid", emb_path="", pretrain_dim=768, use_ed=0, use_mp=0):
        super().__init__()
        """
        Input:
            ...
            use_mp: number of MP features to use (0 or 8)
        """
        self.model_name = "akt"
        self.n_question = n_question
        self.num_c = n_question  # For compatibility with train_model.py
        ...
        # Adjust d_model if using MP features, ensuring divisibility by num_attn_heads
        self.num_attn_heads = num_attn_heads
        if self.num_mp_total_features > 0:
            feature_padding = ((self.num_mp_total_features + num_attn_heads - 1) // num_attn_heads) * num_attn_heads
            effective_d_model = d_model + feature_padding
        else:
            effective_d_model = d_model
        effective_d_feature = effective_d_model // num_attn_heads
        self.model = Architecture(n_question=n_question, n_blocks=n_blocks, n_heads=num_attn_heads, dropout=dropout,
                                    d_model=effective_d_model, d_feature=effective_d_feature, d_ff=d_ff,  kq_same=self.kq_same, model_type=self.model_type, emb_type=self.emb_type)

        # Adjust output layer input size if using MP features
        # concat_q concatenates d_output (effective_d_model) + q_embed_data (also effective_d_model after MP padding)
        out_input_dim = effective_d_model + effective_d_model
        ...
        if self.use_mp == 8:
            ...
            for i in range(4):  # Ratio 0-3 예측 (4개)
                ratio_out = nn.Sequential(
                    nn.Linear(out_input_dim, final_fc_dim), nn.ReLU(), nn.Dropout(self.dropout),
                    nn.Linear(final_fc_dim, 256), nn.ReLU(), nn.Dropout(self.dropout),
                    nn.Linear(256, n_question)  # Output one ratio per concept (same as DKT)
                )
                self.ratio_out_layers.append(ratio_out)
            self.ratio_out_layers = nn.ModuleList(self.ratio_out_layers)

            # Step 2: Ratio 4*n_question개를 입력으로 받아 prediction 예측하는 FC layer
            # Input: out_input_dim + n_question*4 (ratio predictions) -> Output: 1 (prediction)
            self.out = nn.Sequential(
                nn.Linear(out_input_dim + n_question*4, final_fc_dim), nn.ReLU(), nn.Dropout(self.dropout),
                nn.Linear(final_fc_dim, 256), nn.ReLU(), nn.Dropout(self.dropout),
                nn.Linear(256, 1)
            )
        else:
    ...
    def forward(self, q_data, target, pid_data=None, qtest=False, dcur=None):
        ...
        if emb_type.startswith("qid"):
            ...
            # If num_mp_total_features > 0, concatenate features
            if self.num_mp_total_features > 0:
                batch_size, seq_len = q_embed_data.shape[:2]
                feature_padding = ((self.num_mp_total_features + self.num_attn_heads - 1) // self.num_attn_heads) * self.num_attn_heads
                feature_tensor = torch.zeros((batch_size, seq_len, feature_padding), device=q_embed_data.device, dtype=q_embed_data.dtype)

                # Add MP features if available
                if self.use_mp > 0 and mp is not None:
                    feature_tensor[:, :, :self.num_mp_total_features] = mp
        ...
        if self.n_pid > 0:
            ...
            if self.num_mp_total_features > 0:
                batch_size, seq_len = qa_embed_diff_data.shape[:2]
                feature_padding = ((self.num_mp_total_features + self.num_attn_heads - 1) // self.num_attn_heads) * self.num_attn_heads
        ...
        if self.use_mp == 8:
            ...
            for i in range(4):  # Ratio 0-3 예측
                ratio_pred = self.ratio_out_layers[i](concat_q)  # Shape: (batch, seq, n_question)
                ratio_pred = m(ratio_pred)  # [0, 1] range for ratio
                ratio_predictions.append(ratio_pred)

            # Step 2: Concatenate ratio predictions to concat_q
            # ratio_predictions: list of 4 tensors, each (batch, seq, n_question)
            ratio_concat = torch.cat(ratio_predictions, dim=-1)  # Shape: (batch, seq, n_question*4)

            # Step 3: Predict final output using concat_q + ratio predictions
            concat_q_with_ratio = torch.cat([concat_q, ratio_concat], dim=-1)  # Shape: (batch, seq, out_input_dim + n_question*4)
            ...
            if not qtest:
                # Training: return prediction + c_reg_loss + ratio predictions
                return preds, c_reg_loss, ratio_predictions
            else:
