...
def main(params):
    ...
    if params['use_wandb']==1:
        import wandb
        archi = "" if params["archi"] == 1 else "origin_archi"
        del params['archi']
        alpha = params.get('alpha', 1)  # Get alpha value, default to 1
        use_ed_str = "use_ed" if params.get("use_ed", False) else "no_ed"
        use_mp_str = "use_mp" if params.get("use_mp", False) else "no_mp"
        run_name = f"{params['model_name']}_{archi}_ratio_weight {alpha}_dropout {params['dropout']}_lr {params['learning_rate']}_seed {params['seed']}_fold {params['fold']}_ed {params['use_ed']}_mp {params['use_mp']}"
        wandb.init(
            name=run_name,
            group = f"{params['model_name']}_dropout {params['dropout']}_lr {params['learning_rate']}",
            job_type = f"{params['model_name']}_{archi}_ratio_weight {alpha}_{use_ed_str}_{params['use_mp']}"
        )
    ...
    with open("../configs/kt_config.json") as f:
        ...
        for key in ["model_name", "dataset_name", "emb_type", "save_dir", "fold", "seed", "cold_start_len"]:
    ...
    alpha = params.get('alpha', 1.0)  # Get alpha value, default to 1.0
    print(f"alpha: {alpha}")
    for remove_item in ['use_wandb','learning_rate','add_uuid','l2','use_ed', 'use_mp', 'alpha']:
    ...
    use_ed = params.get("use_ed", False)
    use_mp = params.get("use_mp", 0)
    model = init_model(model_name, model_config, data_config[dataset_name], emb_type, use_ed=use_ed, use_mp=use_mp)
    ...
    if model_name == "rkt":
        testauc, testacc, window_testauc, window_testacc, validauc, validacc, best_epoch = \
            train_model(model, train_loader, valid_loader, num_epochs, opt, ckpt_path, None, None, save_model, data_config[dataset_name], fold, alpha=alpha)
    else:
        testauc, testacc, window_testauc, window_testacc, validauc, validacc, best_epoch = train_model(model, train_loader, valid_loader, num_epochs, opt, ckpt_path, None, None, save_model, alpha=alpha)
    
    if save_model:
        best_model = init_model(model_name, model_config, data_config[dataset_name], emb_type, use_ed=use_ed, use_mp=use_mp)
    ...
    if save_model:
        ...
        # Load test dataset
        test_data_config = data_config[dataset_name].copy()
        test_data_config["dataset_name"] = dataset_name
        cold_start_len = params.get("cold_start_len", 0)
        print(f"cold_start_len: {cold_start_len}")
        test_loader, test_window_loader, test_question_loader, test_question_window_loader = init_test_datasets(test_data_config, model_name, batch_size=256, cold_start_len=cold_start_len)
        
        # Set model to evaluation mode and move to device
        best_model.eval()
        best_model.to(device)
        
        # Evaluate on test set with qualitative logging
        final_testauc, final_testacc = evaluate(best_model, test_loader, model_name, log_dir=ckpt_path)

        print(f"Final Test AUC: {final_testauc:.4f}")
        print(f"Final Test Accuracy: {final_testacc:.4f}")
        print("=" * 40)
    ...
    if params['use_wandb']==1:
        ...
        # Add test results to wandb if evaluation was performed
        if save_model:
            wandb_log_dict["testauc"] = final_testauc
            wandb_log_dict["testacc"] = final_testacc
            wandb_log_dict["testavg"] = (final_testauc + final_testacc) / 2
