...
class stableKT(nn.Module):
    def __init__(self, n_question, n_pid, 
            ...
            kq_same=1, final_fc_dim=512, final_fc_dim2=256, num_attn_heads=8, separate_qa=False, l2=1e-5, emb_type="qid", r=1, gamma=1, emb_path="", pretrain_dim=768,num_buckets=32,max_distance=100, use_ed=0, use_mp=0):
        super().__init__()
        """
        Input:
    ...
    def forward(self, dcur, qtest=False, train=False):
        ...
        if self.num_ed_features > 0:
            ...
            for i in range(self.num_ed_features):
                ...
                if ed_key in dcur and len(dcur[ed_key]) > 0:
                    # Shift ED features: current ED should come from previous interactions
                    ed_shifted = torch.cat([torch.zeros(dcur[ed_key].shape[0], 1, device=device), 
                                          dcur[ed_key][:, :-1]], dim=1)
                    ed_data.append(ed_shifted.to(device).float())
                else:
                    ed_data.append(torch.zeros_like(target.float(), device=device))
            
            if len(ed_data) > 0:
                ed = torch.stack(ed_data, dim=-1)  # [batch_size, seq_len, 4]
        ...
        if self.use_mp > 0:
            ...
            for i in range(self.num_mp_meta_features):
                ...
                if mp_key in dcur and len(dcur[mp_key]) > 0:
                    ...
                    mp_data.append(torch.zeros(target.shape[0], target_seq_len, device=device, dtype=torch.float))
        ...
        if emb_type in ["qid", "qidaktrasch", "qid_scalar", "qid_norasch", "qid_sin", "qid_rotary", "qid_t5", "qid_woha", "qid_wha"]:
            ...
            if self.use_mp == 8 and train:
                # Multi-task: Ratio 0-3 predictions (4개) - only during training
                ratio_predictions = []
                for i in range(4):  # Ratio 0-3 예측
                    ratio_pred = self.ratio_out_layers[i](concat_q).squeeze(-1)  # Shape: (batch, seq)
