...
ALL_KEYS = ["fold", "uid", "questions", "concepts", "responses", "timestamps",
            ...
            "math_prof_0", "math_prof_1", "math_prof_2", "math_prof_3",
            "math_prof_4", "math_prof_5", "math_prof_6", "math_prof_7"]
ONE_KEYS = ["fold", "uid"]


def read_data_15_line(fname, min_seq_len=3, response_set=[0, 1]):
    """Read 15-line format data (for custom datasets with error types and math proficiency)"""
    effective_keys = set()
    dres = dict()
    ...
    with open(fname, "r", encoding="utf8") as fin:
        ...
        while i < len(lines):
            ...
            if i % 19 == 0:  # stuid
                ...
                dcur["uid"] = stuid
                goodnum += seq_len
            elif i % 19 == 1:  # question ids / names
                qs = []
                if line.find("NA") == -1:
                    effective_keys.add("questions")
                    qs = line.split(",")
                dcur["questions"] = qs
            elif i % 19 == 2:  # concept ids / names
                cs = []
                if line.find("NA") == -1:
                    effective_keys.add("concepts")
                    cs = line.split(",")
                dcur["concepts"] = cs
            elif i % 19 == 3:  # responses
                ...
                if line.find("NA") == -1:
                    ...
                    if not flag:
                        ...
                        continue
                dcur["responses"] = rs
            elif i % 19 == 4:  # timestamps
                ts = []
                if line.find("NA") == -1:
                    effective_keys.add("timestamps")
                    ts = line.split(",")
                dcur["timestamps"] = ts
            elif i % 19 == 5:  # response_times
                usets = []
                if line.find("NA") == -1:
                    effective_keys.add("usetimes")
                    usets = line.split(",")
                dcur["usetimes"] = usets
            elif i % 19 == 6:  # error_type_0
                error_type_0 = []
                if line.find("NA") == -1:
                    effective_keys.add("error_type_0")
                    error_type_0 = line.split(",")
                dcur["error_type_0"] = error_type_0
            elif i % 19 == 7:  # error_type_1
                error_type_1 = []
                if line.find("NA") == -1:
                    effective_keys.add("error_type_1")
                    error_type_1 = line.split(",")
                dcur["error_type_1"] = error_type_1
            elif i % 19 == 8:  # error_type_2
                error_type_2 = []
                if line.find("NA") == -1:
                    effective_keys.add("error_type_2")
                    error_type_2 = line.split(",")
                dcur["error_type_2"] = error_type_2
            elif i % 19 == 9:  # error_type_3
                error_type_3 = []
                if line.find("NA") == -1:
                    effective_keys.add("error_type_3")
                    error_type_3 = line.split(",")
                dcur["error_type_3"] = error_type_3
            elif i % 19 == 10:  # error_type_4
                error_type_4 = []
                if line.find("NA") == -1:
                    effective_keys.add("error_type_4")
                    error_type_4 = line.split(",")
                dcur["error_type_4"] = error_type_4
            elif i % 19 == 11:  # math_prof_0
                math_prof_0 = []
                if line.find("NA") == -1:
                    effective_keys.add("math_prof_0")
                    math_prof_0 = line.split(",")
                dcur["math_prof_0"] = math_prof_0
            elif i % 19 == 12:  # math_prof_1
                math_prof_1 = []
                if line.find("NA") == -1:
                    effective_keys.add("math_prof_1")
                    math_prof_1 = line.split(",")
                dcur["math_prof_1"] = math_prof_1
            elif i % 19 == 13:  # math_prof_2
                math_prof_2 = []
                if line.find("NA") == -1:
                    effective_keys.add("math_prof_2")
                    math_prof_2 = line.split(",")
                dcur["math_prof_2"] = math_prof_2
            elif i % 19 == 14:  # math_prof_3
                math_prof_3 = []
                if line.find("NA") == -1:
                    effective_keys.add("math_prof_3")
                    math_prof_3 = line.split(",")
                dcur["math_prof_3"] = math_prof_3
            elif i % 19 == 15:  # math_prof_4
                math_prof_4 = []
                if line.find("NA") == -1:
                    effective_keys.add("math_prof_4")
                    math_prof_4 = line.split(",")
                dcur["math_prof_4"] = math_prof_4
            elif i % 19 == 16:  # math_prof_5
                math_prof_5 = []
                if line.find("NA") == -1:
                    effective_keys.add("math_prof_5")
                    math_prof_5 = line.split(",")
                dcur["math_prof_5"] = math_prof_5
            elif i % 19 == 17:  # math_prof_6
                math_prof_6 = []
                if line.find("NA") == -1:
                    effective_keys.add("math_prof_6")
                    math_prof_6 = line.split(",")
                dcur["math_prof_6"] = math_prof_6
            elif i % 19 == 18:  # math_prof_7
                math_prof_7 = []
                if line.find("NA") == -1:
                    effective_keys.add("math_prof_7")
                    math_prof_7 = line.split(",")
                dcur["math_prof_7"] = math_prof_7
    ...
    print(f"delstu: {delstu}, delnum: {delnum}, goodnum: {goodnum}, badr: {badr}")
...
def main(dname, fname, dataset_name, configf, min_seq_len=3, maxlen=200, kfold=5):
    ...
    # Use appropriate read function based on dataset
    if dataset_name in ["custom"]:
        total_df, effective_keys = read_data_15_line(fname)
    else:
