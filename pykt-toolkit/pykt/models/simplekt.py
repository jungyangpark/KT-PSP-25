...
class simpleKT(nn.Module):
    def __init__(self, n_question, n_pid, 
            ...
            kq_same=1, final_fc_dim=512, final_fc_dim2=256, num_attn_heads=8, separate_qa=False, l2=1e-5, emb_type="qid", emb_path="", pretrain_dim=768, use_ed=0, use_mp=0):
        super().__init__()
        """
        Input:
        ...
        # Adjust final layer input size if using ED and MP features
        final_input_dim = d_model + embed_l + self.num_ed_features + self.num_mp_features
        self.out = nn.Sequential(
            nn.Linear(final_input_dim,
        ...
        if self.use_mp == 8:
            ...
            for i in range(4):  # Ratio 0-3 예측 (4개)
                ratio_out = nn.Sequential(
                    nn.Linear(final_input_dim, final_fc_dim), nn.ReLU(), nn.Dropout(self.dropout),
    ...
    def forward(self, dcur, qtest=False, train=False):
        ...
        if emb_type in ["qid", "qidaktrasch", "qid_scalar", "qid_norasch"]:
            ...
            concat_inputs = [d_output, q_embed_data]
            ...
            if self.num_ed_features > 0:
                if ed is not None:
                    ...
                    batch_size, seq_len = d_output.shape[:2]
                    ed_padding = torch.full((batch_size, seq_len, self.num_ed_features), -1.0, device=d_output.device, dtype=d_output.dtype)
            ...
            if self.num_mp_features > 0:
                if mp is not None:
                    ...
                    batch_size, seq_len = d_output.shape[:2]
                    mp_padding = torch.full((batch_size, seq_len, self.num_mp_features), -1.0, device=d_output.device, dtype=d_output.dtype)
                    concat_inputs.append(mp_padding)
            
            concat_q = torch.cat(concat_inputs, dim=-1)
            ...
            if self.use_mp == 8 and not qtest:
                ...
                for i in range(4):  # Ratio 0-3 예측
                    ratio_pred = self.ratio_out_layers[i](concat_q).squeeze(-1)
                    ratio_pred = torch.sigmoid(ratio_pred)
                    ratio_predictions.append(ratio_pred)

                if train:
                    return preds, y2, y3, ratio_predictions
                else:
                    return preds, ratio_predictions
            else:
