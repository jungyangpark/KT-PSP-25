...
class DKTPlus(Module):
    def __init__(self, num_c, emb_size, lambda_r, lambda_w1, lambda_w2, dropout=0.1, emb_type="qid", emb_path="", pretrain_dim=768, use_ed=0, use_mp=0):
        ...
        if self.use_mp == 8:
            ...
            # Input: hidden_size + num_c*4 (ratio predictions) -> Output: num_c (prediction)
            self.out_layer = Linear(self.hidden_size + self.num_c*4, self.num_c)
        else:
    ...
    def forward(self, q, r, dcur=None):
        ...
        if self.use_mp == 8:
            ...
            for i in range(4):  # Ratio 0-3 예측
                ratio_pred = self.ratio_out_layers[i](h)  # Shape: (batch, seq, num_c)
                ratio_pred = torch.sigmoid(ratio_pred)  # [0, 1] range for ratio
                ratio_predictions.append(ratio_pred)

            # Step 2: Concatenate ratio predictions to h
            ...
            # Step 3: Predict final output using h + ratio predictions
            h_with_ratio = torch.cat([h, ratio_concat], dim=-1)  # Shape: (batch, seq, hidden_size + num_c*4)
            y = self.out_layer(h_with_ratio)  # Shape: (batch, seq, num_c)
            y = torch.sigmoid(y)
